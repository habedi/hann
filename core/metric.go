...
// requirement, so the three always travel together. Stats reports the name,
// and the serialized form of an index stores it. The normalization flag
// replaces the old convention of switching behavior on the name "cosine".
type Metric struct {
	name      string
	fn        DistanceFunc
	normalize bool
	rankFn    DistanceFunc          // orders like fn, but may be cheaper
...
// vector in a flat buffer. It writes one value per candidate into out. The
// caller has already checked the lengths.
...
// Distance itself. Rank values from different metrics are not comparable.
// FromRank converts a rank value to the true distance.
...
// vectors. It writes one distance per candidate into out. The candidates are
// stored one after another in flat, one row of len(query) floats per
// candidate, so n is len(flat) divided by len(query). It returns an error
// when the query is empty, when len(flat) is not a multiple of len(query),
// or when len(out) differs from n. With n equal to zero it is a no-op. A
// built-in metric computes the batch in one C call. A metric created with
// NewMetric falls back to a loop over Distance.
...
// vectors. It writes one value per candidate into out. It relates to
// DistanceBatch exactly as Rank relates to Distance: the values order
// candidates the same way but may be cheaper, and FromRank converts them to
// true distances. The arguments follow the same flat layout and the same
// validation as DistanceBatch.
...
// batchSize checks the flat-layout arguments shared by DistanceBatch and
// RankBatch. It returns the number of candidate vectors.
...
// per-pair function. It serves metrics without a batch kernel.
...
// RegisterMetric adds a custom metric to the registry, so an index saved
// with it can be loaded by name. It returns an error when the metric has no
// name or no distance function, or when the name is already registered.
